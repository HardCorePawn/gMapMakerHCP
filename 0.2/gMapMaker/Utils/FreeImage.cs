...
//#endif
		...
		[DllImport(dllName, EntryPoint="FreeImage_ColorQuantizeEx")]
		private static extern FIBITMAP ColorQuantizeEx(FIBITMAP dib, FREE_IMAGE_QUANTIZE quantize, int PaletteSize, int ReserveSize, UIntPtr ReservePalette);

		public static unsafe FIBITMAP ConvertPalette(FIBITMAP srcPaletteDib, FIBITMAP dib)
		{
			UIntPtr srcPalette = GetRawPalette(srcPaletteDib);
			UIntPtr palette = GetRawPalette(dib);
			uint[] mapping = new uint[256];
			byte* srcPtr = (byte*)(void*)srcPalette;
			byte* ptr = (byte*)(void*)palette;
			uint j;
			/*
      int i;
      for (i = 0; i < 256; i++, ptr += 4) 
			{
        byte* sPtr = srcPtr;
        for (j = 0; j < 256; j++, sPtr += 4) 
				{
          if ((*ptr == *sPtr) && (*(ptr + 1) == *(sPtr + 1)) && (*(ptr + 2) == *(sPtr + 2)))
					{
						mapping[ i ] = j;
						break;
					}
			...
			FIBITMAP dib8 = Clone(dib);
			if (dib8 == 0)
			{
				Unload(dib8);
				return 0;
			}
			if (!PaletteCopy(srcPaletteDib, dib8))
			{
				Unload(dib8);
				return 0;
			}
			...
			for (uint y = 0; y < height; y++)
			{
				...
				{
					byte c = 0;
					if (!GetPixelIndex(dib, x, y, ref c))
					{
						Unload(dib8);
						return 0;
					}
					//c = (byte)mapping[ c ];
          byte* sPtr = (byte*)(void*)srcPalette;
          ptr = (byte*)(void*)palette;
          byte r = *(ptr + c * 4 + 0);
          byte g = *(ptr + c * 4 + 1);
          byte b = *(ptr + c * 4 + 2);
          byte a = *(ptr + c * 4 + 3);
          ...
          if (!SetPixelIndex(dib8, x, y, ref c))
					{
						Unload(dib8);
						return 0;
					}
		...
		*/
		...
		public static unsafe bool PaletteCopy(FIBITMAP srcDib, FIBITMAP destDib)
		{
			// Only interested in indexed images. 
			if ((GetBPP(srcDib) <= 8) && (GetBPP(destDib) <= 8))
			{
				byte* srcPtr = (byte*)(void*)GetRawPalette(srcDib);
				byte* destPtr = (byte*)(void*)GetRawPalette(destDib);
				if ((srcPtr == null) || (destPtr == null))
				{
					return false;
				}
				for (int q = 0; q < 256*4; q++, srcPtr++, destPtr++)
				{
